...
class dataset(object):
    ...
    def pooling(self, data, stride, batch):
        ...
        if self.conv2d:
            pooling_imgs = np.reshape(pooling_imgs[:, -1, :, :, :],
                                      (-1, 101, 101, l))
    ...
    def data_generateor(self,
        ...
        while True:
            try:
                batch_img = []
                if cate == 'training':
                    batch_path = self.train_img_path[bt * batch:(bt + 1) *
                                                     batch]
                    bt += 1
                ...
                batch_img_0, batch_img_1, batch_img_2, batch_img_3, label_img, id_img = self.data_sep(
                    batch_img, batch=batch)

                if rand_h:
                    imgs_h = self.random_height(
                        [batch_img_0, batch_img_1, batch_img_2, batch_img_3],
                        rand_h_num)
                    ...
                    if height == 0:
                        batch_img_0 = self.pooling(
                            batch_img_0, pooling_stride, batch=batch)
                        #print('batch_img_0 selected!')
                        yield (batch_img_0, label_img)
                    elif height == 1:
                        batch_img_1 = self.pooling(
                            batch_img_1, pooling_stride, batch=batch)
                        #print('batch_img_1 selected!')
                        yield (batch_img_1, label_img)
                    elif height == 2:
                        batch_img_2 = self.pooling(
                            batch_img_2, pooling_stride, batch=batch)
                        #print('batch_img_2 selected!')
                        yield (batch_img_2, label_img)
                    elif height == 3:
                        batch_img_3 = self.pooling(
                            batch_img_3, pooling_stride, batch=batch)
                        #print('batch_img_2 selected!')
...
if __name__ == '__main__':
    ...
    batch_training_generator = train_dataset.data_generateor(
        'training', rand_h=True, rand_h_num=4, height=3, pooling_stride=3)
    batch_val_generator = train_dataset.data_generateor(
        'val', rand_h=True, rand_h_num=4, height=3, pooling_stride=3, batch=10)
    for i in range(20):
