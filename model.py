...
def log(text, array=None):
    ...
    if array is not None:
        ...
        text += ("shape: {:20}  min: {:10.5f}  max: {:10.5f}  {}".format(
            ...
            array.max() if array.size else "", array.dtype))
    print(text)
...
class models(object):
    ...
    def find_last(self):
        ...
        checkpoints = list(
            filter(lambda f: f.startswith("model"), checkpoints))
    ...
    def set_log_dir(self, model_path=None):
        ...
        if self.mode == 'training':
            ...
            self.log_dir = os.path.join(
                self.model_dir, "{}{:%Y%m%dT%H%M}".format(self.name, now))
            ...
            self.checkpoint_path = os.path.join(
                self.log_dir, "{}_*epoch*.h5".format(self.name))
...
if __name__ == "__main__":
    ...
    dic = Config.dic
    #训练数据生成器
    train_img_generator = dataset.data_generateor(
        'training',
        rand_h=dic['rand_h'],
    ...
    #验证数据生成器
    val_img_generator = dataset.data_generateor(
        'val',
        rand_h=dic['rand_h'],
        rand_h_num=dic['rand_h_num'],
        height=dic['height'],
